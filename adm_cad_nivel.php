...
if(isset($_POST['btnenviar'])){		
		...
		$sql="insert into tbltipouser (nometipo) 
		values('".$titulo."')";
		}elseif($operacao=="Editar"){
			$sql="update tbluser set nomeuser='".$titulo."' where codtipo='".$_SESSION["cod"]."'";

			//echo($sql);
		}
		mysql_query($sql);
		header("location:adm_all_nivel.php");
	}
	if(isset($_GET["modo"])){
		$modo=$_GET["modo"];
	if($modo=="editar"){

			$_SESSION["cod"]=$_GET["codtipo"];
			$sql="select * from tbltipouser where codtipo=".$_SESSION["cod"];
