...
mysql_select_db("dblivraria");
	...
	$modo=$_GET["modo"];
		if ($modo=="excluir"){
			$cod = $_GET["codsubcat"];
			$sql="delete from tblsubcategoria where codsubcat=".$cod;
...
<body>
    ...
    <div class="principal">
		...
		<div class="consulta">
			...
			<?php
				$sql="select s.codsubcat, s.nomesubcat, c.nomecat
						from tblsubcategoria as s
						inner join tblcategoria as c
						on (s.codcat = c.codcat)
						order by codsubcat desc";
			...
			<tr height="30px" bgcolor="<?php if($linha % 2){ echo "#D8DFFA"; } else { echo "#FFFFFF"; } ?>">
				<td width="200" >
					<?php echo($rs["nomesubcat"])?>
				</td>
				<td width="200">
					<?php echo($rs["nomecat"])?>
				</td>
				<td width="200" align="center">
					<a href="adm_cad_subcategoria.php?modo=editar&codsubcat=<?php echo($rs["codsubcat"]) ?>"> Editar </a> | <a href="adm_all_subcategoria.php?modo=excluir&codsubcat=<?php echo($rs["codsubcat"]) ?>">Excluir</a>
				</td>
			</tr>
			<?php
					$linha++;
